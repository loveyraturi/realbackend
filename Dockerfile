...
# # Tag praveen/file_versioning:v1
# ############################################################
...
RUN git clone https://github.com/loveyraturi/file_versioning.git
WORKDIR /file_versioning
RUN mvn clean install -DskipTests 
RUN mv target/${APP_NAME}-${APP_VERSION}.war /tasksvn.war
...
COPY --from=WAR /tasksvn.war /opt/tomcat/webapps/
